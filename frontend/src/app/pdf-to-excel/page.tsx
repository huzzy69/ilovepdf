...
export default function PdfToExcelPage() {
  ...
  const handleConvert = async (files: File[]) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/convert/pdf-to-excel`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white flex flex-col p-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-3xl w-full mx-auto text-center space-y-8 pb-12">
        <h1 className="text-5xl font-bold text-emerald-400">PDF to Excel</h1>
        <p className="text-xl text-neutral-400">Pull data straight from PDFs into Excel spreadsheets.</p>
        
        <div className="flex justify-center w-full">
          <FileUpload onFilesSelected={handleConvert} title="Select PDF" />
        </div>
        {isProcessing && <p className="text-emerald-400 animate-pulse mt-4">Converting document...</p>}
      </div>
    </div>
  );
}
