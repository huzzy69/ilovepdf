...
export default function CompressPdfPage() {
  ...
  const handleCompress = async (files: File[]) => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/convert/compress`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen bg-neutral-900 text-white flex flex-col p-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center max-w-3xl w-full mx-auto text-center space-y-8 pb-12">
        <h1 className="text-5xl font-bold text-green-400">Compress PDF</h1>
        <p className="text-xl text-neutral-400">Reduce file size while optimizing for maximal PDF quality.</p>
        
        <div className="flex justify-center w-full">
          <FileUpload onFilesSelected={handleCompress} title="Select PDF" />
        </div>
        {isProcessing && <p className="text-green-400 animate-pulse mt-4">Compressing document...</p>}
      </div>
    </div>
  );
}
